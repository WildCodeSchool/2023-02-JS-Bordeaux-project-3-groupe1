...
const getTutorialByIconFormationNoId = async (userId) => {
 ...
 LEFT JOIN (SELECT
   ...
   GROUP BY tutorials.formation_id, level, user_id) AS total_steps_by_user_true ON total_steps_by_user_true.user_id = user_tuto.user_id AND total_steps_by_user_true.formation_id = formations.id and total_steps_by_user_true.level = tutorials.level`,
      [userId]
    );
    return rows[0];
  } catch (error) {
